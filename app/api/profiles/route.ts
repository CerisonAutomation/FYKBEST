...
import { logApiError, logApiRequest } from '@/lib/logging/api-logger'
...
export const GET = withRateLimit(getProfiles, 'api')
export const POST = withRateLimit(createProfile, 'api')
...
export const PATCH = withRateLimit(async function updateProfile(request: Request) {
  ...
  try {
    ...
    if (authError || !user) {
      await logApiError(request, 'internal_error', authError)
      return setSecurityHeaders(
        NextResponse.json(
          { error: 'Internal server error', code: 'AUTH_REQUIRED' },
          { status: 401 }
        )
      )
    }
    ...
    if (!validatedData.success) {
      await logApiError(request, 'internal_error', validatedData.error)
      return setSecurityHeaders(
        NextResponse.json(
          {
            error: 'Internal server error',
    ...
    if (updateData.username) {
      ...
      if (existingProfile) {
        await logApiError(request, 'internal_error', { username: updateData.username })
        return setSecurityHeaders(
          NextResponse.json(
            { error: 'Internal server error', code: 'USERNAME_TAKEN' },
            { status: 409 }
          )
        )
      }
    ...
    if (error) {
      await logApiError(request, 'internal_error', error)
      return setSecurityHeaders(
        NextResponse.json(
          {
            error: 'Internal server error',
    ...
    await logApiError(request, 'internal_error', error)
    return setSecurityHeaders(
      NextResponse.json({ error: 'Internal server error', code: 'INTERNAL_ERROR' }, { status: 500 })
    )
  }
}, 'api')
